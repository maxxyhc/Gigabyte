...
3. **兩種檢索器單獨使用皆不足。** dense 的 R@3 為 0.986、BM25 為 0.891，
   ...
   換取「不再整題落空」。以 top-3 餵給模型的用法而言，後者較重要。
...
5. **規格總覽 chunk 目前未展現效益**（移除後 MRR 反而由 0.928 微升至 0.935）。
   但評測集中**沒有一題是「這台有什麼特色」這類籠統問題**，而那正是該 chunk
   存在的理由，因此這個數字反映的是評測覆蓋不足。
...
2. **無 RAG 的失效模式不是「答不出來」，是「有自信地編造」。**
   ...
   AM6H 發表於模型訓練資料之後，缺乏 grounding 時模型會以合理的語氣填補空白，
   並附上「建議至官方網站查詢最新資訊」這類使人更易採信的措辭。
